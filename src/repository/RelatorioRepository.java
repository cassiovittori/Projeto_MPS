...
public class RelatorioRepository {
    ...
    public static Relatorio saveRelatorio(Relatorio relatorio) throws SQLException {
        try (Connection connection = DriverManager.getConnection(Constantes.URL_BANCO)) {
            String sql = "INSERT INTO relatorios (cabecalho, corpo,rodape, data_criacao, autor) VALUES (?, ?,?, ?, ?)";
            try (PreparedStatement statement = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                statement.setString(1, relatorio.getCabecalho());
                statement.setString(2, relatorio.getCorpo());
                statement.setString(3, relatorio.getRodape());
                statement.setString(4, relatorio.getDataCriacao());
                statement.setString(5, relatorio.getAutor());
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    if (resultSet.next()) {
                        relatorio = new Relatorio(
                            resultSet.getString("cabecalho"),
                            resultSet.getString("corpo"),
                            resultSet.getString("rodape"),
